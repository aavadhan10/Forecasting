...
def load_time_entries() -> pd.DataFrame:
    """
    Load and clean the Time Entry prep files.
    This assumes the files are accessible to the app process; no upload needed.
    """
    files = [
...
def load_invoice_prep() -> pd.DataFrame:
    ...
    """
    try:
        df = pd.read_excel("Invoice Prep File (10.31).xlsx")
    except FileNotFoundError:
        return pd.DataFrame()
    ...
    # Some useful numeric columns we might use for KPIs
    for col in [
        "Original Inv. Total",
        "Orig Labor Total",
        "Orig Expense Total",
...
def load_payment_prep() -> pd.DataFrame:
    """
    Load and clean Payment Prep file.
    """
    try:
        df_raw = pd.read_excel("Payment Prep File (10.31).xlsx")
    except FileNotFoundError:
        return pd.DataFrame()
...
def prepare_monthly_time_by_rate(df: pd.DataFrame) -> pd.DataFrame:
    """
    Aggregates monthly billable amounts by rate type (hourly, flat, etc.).
    Returns a long-format dataframe with:
      - YearMonth (period month as timestamp)
      - Rate_Type
      - Billable_Amount_in_USD
    """
    if df.empty:
...
def simple_linear_forecast(series: pd.Series, periods: int = 3) -> pd.Series:
    """
    Very simple linear forecast using numpy.polyfit on the index positions.
    Returns a future series of length `periods`.
    """
...
def generate_trend_summary(monthly_long: pd.DataFrame) -> str:
    """
    Generates a short narrative of key trends, especially around flat fees.
    """
    if monthly_long.empty:
        return "No data available for trend analysis."

    # Pivot for easier per-rate analysis
    pivot = (
        monthly_long
    ...
    if flat_col is not None:
        ...
        # Last two months comparison (for the 'October vs September' style insight)
        if len(flat_share) >= 2:
...
def main():
    ...
    if time_df.empty:
        st.error("Could not load Time Entry prep files. Check file names and paths.")
        st.stop()
    ...
    # Apply global filters (based on time entries)
    filtered_time = apply_time_entry_filters(time_df)

    # Compute common derived data
    monthly_long = prepare_monthly_time_by_rate(filtered_time)

    # Identify flat-fee vs other for KPIs
    ...
    if page == "KPI Overview":
        ...
        if not monthly_long.empty:
            ...
            # Share of flat-fee (if detectable)
        ...
        if monthly_long.empty:
            ...
            # Forecast for mix (per rate type)
